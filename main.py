...
async def change_password(password_data: schemas.PasswordChange, db: Session = Depends(get_db),
                          current_user: tablesmodel.User = Depends(oAuth2.get_current_user)):
    if not current_user:
...
async def get_student_details(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
...
async def get_professor_details(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                                db: Session = Depends(get_db)):
...
async def get_attendance_list(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                              db: Session = Depends(get_db)):
    professor = db.query(tablesmodel.Professor).filter(tablesmodel.Professor.user_id == current_user.id).first()
    if not professor:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Professor not found")
    ...
    attendance_list = [{"name": student.name, "roll_number": student.roll_number, "present": False} for student in
                       students]
...
async def submit_attendance(attendance_list: list[dict],
                            current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                            db: Session = Depends(get_db)):
    if not current_user:
        ...
        try:
            ...
            if existing_attendance:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                    detail=f"Attendance for Roll Number {roll_number} in Lecture {lecture.id} on {attendance_date} already exists")

            attendance = tablesmodel.Attendance(student_roll_number=roll_number, lecture_id=lecture.id, present=present,
...
async def get_student_attendance(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                                 db: Session = Depends(get_db)):
    student = db.query(tablesmodel.Student).filter(tablesmodel.Student.user_id == current_user.id).first()
    if not student:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Student not found")
    ...
    section_id = db.query(tablesmodel.Section.id).filter(
        tablesmodel.Section.section_name == student.admission_class).first()
    if not section_id:
...
# timetable of exams
@app.get("/datesheet")
async def get_datesheet(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                        db: Session = Depends(get_db)):
    ...
    if not user_detail:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"branch and current semester not found for email {user_email}")

    datesheet = db.query(tablesmodel.DateSheet).filter(tablesmodel.DateSheet.branch == user_detail.branch_name,
                                                       tablesmodel.DateSheet.current_semester == user_detail.current_semester).all()
...
@app.get("/timetable")
async def get_timetable(current_user: tablesmodel.User = Depends(oAuth2.get_current_user),
                        db: Session = Depends(get_db)):
    user_email = db.query(tablesmodel.User.email).filter(tablesmodel.User.id == current_user.id).scalar()
    ...
    timetable_data = db.query(tablesmodel.Timetable).filter(
        tablesmodel.Timetable.branch_name == user_detail.branch_name,
        tablesmodel.Timetable.semester == user_detail.current_semester).all()
...
async def get_result(current_user: tablesmodel.User = Depends(oAuth2.get_current_user), db: Session = Depends(get_db)):
    ...
    if not roll_number:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"roll number not found for email {user_email}")
